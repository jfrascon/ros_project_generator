...
`ros-project` creates a ROS 2 workspace skeleton and can optionally add a VS Code Dev Container
...
`ros-project` owns project structure:
...
```sh
git clone --recurse-submodules git@gitlab.local.eurecat.org:robotics-automation/ros-project.git
cd ros-project
```
...
```sh
python3 -m venv /tmp/ros-project-venv
. /tmp/ros-project-venv/bin/activate
...
The tests mock the Docker generation call where appropriate. They verify that `ros-project` delegates Docker
